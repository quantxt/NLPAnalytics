...
public class PartialAlignment {
	...
	public boolean isUsed(Match m) {
		for (int i = 0; i < m.getLength(); i++)
			if (line2UsedWords[m.getStart() + i])
				return true;
		for (int i = 0; i < m.getMatchLength(); i++)
			if (line1UsedWords[m.getMatchStart() + i])
				return true;
		return false;
	}
	...
	public void setUsed(Match m, boolean b) {
		for (int i = 0; i < m.getLength(); i++)
			line2UsedWords[m.getStart() + i] = b;
		for (int i = 0; i < m.getMatchLength(); i++)
			line1UsedWords[m.getMatchStart() + i] = b;
	}
}
