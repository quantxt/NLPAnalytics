...
public class TERcalc {
    ...
    private static void FindAlignErr(TERalignment align,
                                     boolean[] herr,
                                     boolean[] rerr,
                                     int[] ralign) {
        int hpos = -1;
	...
	*/
    ...
    private Object[] PerformShift(Comparable[] words, int start, int end, int newloc) {
        ...
        if (newloc == -1) {
            for (int i = start; i <= end; i++) {
                nwords[c++] = words[i];
                if (hypSpans != null) spans[c - 1] = hypSpans[i];
            }
    ...
    private TERalignment MinEditDist(Comparable[] hyp, Comparable[] ref,
        ...
        for (int i = 0; i < hyp.length; i++) {
            norm_hyp[i] = hyp[i].toString().replaceAll("[\\,\\.\\;\\:\\-]", "");
        }
        for (int i = 0; i < ref.length; i++) {
            norm_ref[i] = ref[i].toString().replaceAll("[\\,\\.\\;\\:\\-]", "");
        }
            ...
            for (i = first_good; i <= ref.length; i++) {
                ...
                if ((i < ref.length) && (j < hyp.length)) {
                    if (refSpans == null || hypSpans == null ||
                        ...
                        if (norm_ref[i].equals(norm_hyp[j])) {
